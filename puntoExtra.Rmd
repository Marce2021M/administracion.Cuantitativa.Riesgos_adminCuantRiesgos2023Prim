...
varFuncCornisFisher <- function(vectorDatos){
    skewAux <- skewness(vectorDatos)
    curtAux <- kurtosis(vectorDatos, method = "excess")
    cuantNormal <- qnorm(cuantilProb)
    cornisFisher <-
    volAux <-
    varAux <-
...
varHistCornisFisher <- rollapply(data=data$rend,width= window_size, FUN=varFuncCornisFisher, align = "right")

#agregamos columna VaR histórico paramétrico

data$varHistCornisFisher <- c(rep(NA, window_size -1), -varHistCornisFisher*100)
